<template>
	<view>
		<view class="divHead">
			<image src="/static/index_image//background.png" mode="" style="height: 100%;width: 100%;"></image>
		   <image @click="goBack" src="/static/index_image/reback.png" class="back-button" />
		</view>
		
		<view class="divTitle">
			<view class="divStatic">
				<image :src="store.logo" class="logo"></image>
				<view class="divDesc">
					<view class="divName">{{store.storeName}}</view>
					<view class="divSend">
						<image src="/static/index_image/dingwei.png" />
						<text>距离{{store.distance}}km</text>
						<image src="/static/index_image/qiandai.png" />
						<text>配送费{{store.deliveryPrice}}元</text>
						<image src="/static/index_image/shijian.png" />
						<text>预计时长{{store.deliveryTime}}min</text>
					</view>
				</view>
			</view>
			<view class="divDesc">{{store.slogan}}</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			async initData() {
				...
				Promise.all([categoryListApi(storeId), cartListApi({})]).then(res => {
					//获取分类数据
